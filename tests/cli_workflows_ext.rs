...
fn knots_binary() -> PathBuf {
    ...
    if manifest_relative.exists() {
        return std::fs::canonicalize(&manifest_relative)
            .unwrap_or(manifest_relative);
    }
...
fn custom_workflow_supports_non_ready_queue_names() {
    ...
    let home =
        unique_workspace("knots-cli-workflow-generic-queues-home");
    std::fs::create_dir_all(root.join(".knots"))
        .expect(".knots dir should exist");
    let db = root.join(".knots/cache/state.sqlite");
    let bundle_path = root.join("alt-flow.toml");
    std::fs::write(&bundle_path, ALT_QUEUE_NAMES_BUNDLE)
    ...
    let created =
        run_knots(&root, &db, &home, &["new", "Alt workflow knot"]);
    assert_success(&created);
    let knot_id = parse_created_id(&created);

    let shown =
        run_knots(&root, &db, &home, &["show", &knot_id, "--json"]);
    assert_success(&shown);
    let shown_json: Value =
        serde_json::from_slice(&shown.stdout).expect("show json");
    assert_eq!(shown_json["state"], "triage");

    assert_claim_and_advance(&root, &db, &home, &knot_id);
}

fn assert_claim_and_advance(
    root: &Path,
    db: &Path,
    home: &Path,
    knot_id: &str,
    ...
    let claim =
        run_knots(root, db, home, &["claim", knot_id, "--json"]);
    assert_success(&claim);
    let claim_json: Value =
        serde_json::from_slice(&claim.stdout).expect("claim json");
    ...
    assert_success(&next);
    let next_json: Value =
        serde_json::from_slice(&next.stdout).expect("next json");
    ...
    let claim_review_json: Value =
        serde_json::from_slice(&claim_review.stdout)
            .expect("claim review json");
    ...
    let after_json: Value =
        serde_json::from_slice(&after.stdout).expect("show json");
