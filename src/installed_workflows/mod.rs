...
impl PromptDefinition {
    pub fn render(&self, workflow: &WorkflowDefinition, profile: &ProfileDefinition) -> String {
        let resolved_output = profile.step_metadata_for(&self.action_state).output;
        let params = build_prompt_params(&workflow.id, &profile.id, resolved_output.as_ref(), self);
        let template =
