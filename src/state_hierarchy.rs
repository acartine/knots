...
pub fn find_terminal_parent_resolutions(
    ...
    for parent_id in child_graph.keys() {
        if let Some(resolution) = check_parent_resolution(conn, &child_graph, parent_id)? {
...
pub fn find_ancestor_terminal_resolutions(
    ...
    let child_graph = load_child_graph(conn)?;
    let parent_graph = invert_to_parent_graph(&child_graph);
    ...
    while let Some(parent_id) = queue.pop() {
        ...
        if let Some(resolution) = check_parent_resolution(conn, &child_graph, &parent_id)? {
            for ancestor in parent_graph.get(&parent_id).into_iter().flatten() {
