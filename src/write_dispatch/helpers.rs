...
/// Strict lease validation for `next`: if the knot has a bound lease the
/// caller **must** supply the matching lease_id.
pub(crate) fn validate_next_lease(
    ...
    match (knot.lease_id.as_deref(), lease_id) {
        ...
        (None, Some(provided)) => Err(AppError::InvalidArgument(format!(
            "knot has no active lease but caller provided '{provided}'"
        ))),
        (Some(_knot_lease), None) => Err(AppError::InvalidArgument(
